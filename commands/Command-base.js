      ...
      const developer = '𝙈𝙍. 𝙐𝙉𝙆𝙉𝙊𝙒𝙉#6969'
        ...
        for (const permission of permissions) {
          if (!member.permissions.has(permission)) {
            ...
            .setTitle('Error!!')
            .setDescription(`${permissionError} ${permission}`)
            .setTimestamp()
            .setColor('RED')
            ...
            return
          } else if(!guild.me.permissions.has(permission))
          {
              const permembed = new MessageEmbed()
              .setTitle('Error!!')
              .setDescription(`I don't have required permission ${permission}, kindly grant me and try again.`)
              .setTimestamp()
              .setColor('RED')
              .setThumbnail(client.user.displayAvatarURL({dynamic : true}))
              .setFooter(`MADE BY ${developer}` , `${message.author.displayAvatarURL({dynamic : true})}`)
